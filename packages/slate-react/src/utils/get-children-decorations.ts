...
function orderChildDecorations(node, decorations) {
  ...
  keyOrders.set(node.key, 0);
  let globalOrder = 1;

  node.forEachDescendant((child) => {
    keyOrders.set(child.key, globalOrder);
  ...
  decorations.forEach((decoration) => {
    ...
    const startKeyOrder = keyOrders.get(decoration.start.key);
    ...
    // Range end.
    const endKeyOrder =
      (keyOrders.get(decoration.end.key) || globalOrder) + 0.5;
...
function getContainingChildOrder(children, keyOrders, order) {
  ...
  const nextChildIndex = children.findIndex(
    (child) => order < keyOrders.get(child.key)
  );
  ...
  return keyOrders.get(containingChild.key);
}
