...
class Range extends Record(DEFAULTS) {
    ...
    static fromJSON(object: any): Range {
        ...
        if (
            !focus &&
            (object.focusKey || object.focusOffset || object.focusPath)
        ) {
            logger.deprecate(
                "0.37.0",
    ...
    toJSON(options: any = {}): any {
        const object: any = {
            ...
            marks:
                this.marks == null
                    ? null
                    : this.marks.toArray().map(m => m.toJSON())
        };
    ...
    hasEdgeAtStartOf(node) {
        ...
        return (
            this.anchor.isAtStartOfNode(node) ||
            this.focus.isAtStartOfNode(node)
        );
    }
    ...
    hasEdgeAtEndOf(node) {
        ...
        return (
            this.anchor.isAtEndOfNode(node) || this.focus.isAtEndOfNode(node)
        );
    }
